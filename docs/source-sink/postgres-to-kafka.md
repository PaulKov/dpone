...
This guide is a copy/paste-ready starting point for loading data from **Postgres** into **Kafka** with `dpone`.

**Status:** Batch/event-log supported (wide vendor-live certified)

This route uses Postgres **CSV** export into existing `KafkaSink` JSON produce
(optional `envelope: dpone`). Do **not** set `export_format: mssql-delimited` or
`compress_export: true` for this sink (fail-closed). No dedicated PairTypeMatrix
profile (parity with [MySQL -> Kafka](mysql-to-kafka.md)). Wide live
certification asserts typed JSON message payloads from CSV wire — not sink DDL —
for `full_refresh`, `incremental_append`, `incremental_merge`, `replace`,
`snapshot_diff`, and `backfill` (inner `incremental_merge`) via
`tests/integration/postgres/test_postgres_to_kafka_vendor_live_integration.py`.
`partition_replace` / `scd2` and other backfill inner modes are **N/A**. See
...
Use this path when Postgres is the system of record or ingestion boundary and Kafka is the landing, warehouse, event-log, or downstream replication target.
...
defaults:
  name: postgres_to_kafka_example
  source:
    type: postgres
...
```bash
dpone plan examples/source-sink/postgres-to-kafka.yaml --format md
dpone run examples/source-sink/postgres-to-kafka.yaml
```

The checked source file is `examples/source-sink/postgres-to-kafka.yaml`; CI
...
their state advances only after Kafka delivery succeeds. Certify the exact CDC
...
pip install "dpone[kafka,postgres]"
dpone plan examples/source-sink/postgres-to-kafka.yaml --format md
dpone schema type-matrix --source postgres --sink kafka --format md
dpone run examples/source-sink/postgres-to-kafka.yaml
```

Landing convention (vault/GitOps-oriented): `examples/batch/landing_postgres_to_kafka.batch.yaml`.
...
2. Run `dpone plan examples/source-sink/postgres-to-kafka.yaml --format md` and review source boundary, publication and delivery path, schema evolution, state, and quality gates.
3. Run a small bounded window first.
4. Inspect the run artifact under `.dpone/runs/postgres_to_kafka`.
