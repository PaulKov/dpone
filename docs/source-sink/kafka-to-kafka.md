...
```bash
dpone plan examples/source-sink/kafka-to-kafka.yaml --format md
dpone run examples/source-sink/kafka-to-kafka.yaml
```

The checked source file is `examples/source-sink/kafka-to-kafka.yaml`; CI
...
4. Inspect the run artifact under `.dpone/runs/kafka_to_kafka`.
