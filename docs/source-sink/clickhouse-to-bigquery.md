...
```bash
dpone plan examples/source-sink/clickhouse-to-bigquery.yaml --format md
dpone run examples/source-sink/clickhouse-to-bigquery.yaml
```

The checked source file is
`examples/source-sink/clickhouse-to-bigquery.yaml`; CI compares its parsed YAML
with this block.
