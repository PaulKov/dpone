...
This guide is a copy/paste-ready starting point for loading data from **REST API** into **BigQuery** with `dpone`.

## When to use this path

Use this path when REST API is the system of record or ingestion boundary and BigQuery is the landing, warehouse, event-log, or downstream replication target.
...
```bash
dpone plan examples/source-sink/api-to-bigquery.yaml --format md
dpone run examples/source-sink/api-to-bigquery.yaml
```

The checked source file is `examples/source-sink/api-to-bigquery.yaml`; CI
...
4. Inspect the run artifact under `.dpone/runs/api_to_bigquery`.
