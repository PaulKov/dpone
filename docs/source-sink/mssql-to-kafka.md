...
Vendor-live evidence uses a **wide** typed fixture (`dpone_src` CSV → Kafka JSON
envelope) and covers Docker strategies `full_refresh`, `incremental_append`,
`incremental_merge`, `replace`, `snapshot_diff`, and `backfill` (inner
`incremental_merge`) via
`tests/integration/mssql/test_mssql_to_kafka_vendor_live_integration.py`.
`partition_replace` / `scd2` and non-merge backfill inner modes are N/A for
KafkaSink. Extract uses CSV (not `mssql-delimited`). See
...
The checked source file is `examples/source-sink/mssql-to-kafka.yaml`; CI
...
Wide vendor-live covers the Event publication rows below; N/A rows are
documented and not claimed as certified.
...
offsets and advances source state only after Kafka delivery succeeds; certify
the exact route and environment before enabling it.
...
2. Run `dpone plan examples/source-sink/mssql-to-kafka.yaml --format md` and review source boundary, publication and delivery path, schema evolution, state, and quality gates.
