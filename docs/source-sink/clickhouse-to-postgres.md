...
This guide is a copy/paste-ready starting point for loading data from **ClickHouse** into **Postgres** with `dpone`.

## When to use this path

Use this path when ClickHouse is the system of record or ingestion boundary and Postgres is the landing, warehouse, event-log, or downstream replication target.
...
defaults:
  name: clickhouse_to_postgres_example
  source:
    type: clickhouse
...
```bash
dpone plan examples/source-sink/clickhouse-to-postgres.yaml --format md
dpone run examples/source-sink/clickhouse-to-postgres.yaml
```

The checked source file is
`examples/source-sink/clickhouse-to-postgres.yaml`; CI compares its parsed YAML
with this block.
...
4. Inspect the run artifact under `.dpone/runs/clickhouse_to_postgres`.
