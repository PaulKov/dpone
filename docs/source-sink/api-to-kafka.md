...
This guide is a copy/paste-ready starting point for loading data from **REST API** into **Kafka** with `dpone`.

## When to use this path

Use this path when REST API is the system of record or ingestion boundary and Kafka is the landing, warehouse, event-log, or downstream replication target.
...
```bash
dpone plan examples/source-sink/api-to-kafka.yaml --format md
dpone run examples/source-sink/api-to-kafka.yaml
```

The checked source file is `examples/source-sink/api-to-kafka.yaml`; CI
...
strategy; it advances only after Kafka delivery succeeds.
...
4. Inspect the run artifact under `.dpone/runs/api_to_kafka`.
