...
```bash
dpone plan examples/source-sink/api-to-mssql.yaml --format md
dpone run examples/source-sink/api-to-mssql.yaml
```

The checked source file is `examples/source-sink/api-to-mssql.yaml`; CI
...
4. Inspect the run artifact under `.dpone/runs/api_to_mssql`.
