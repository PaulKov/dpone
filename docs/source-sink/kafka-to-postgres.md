...
This guide is a copy/paste-ready starting point for loading data from **Kafka** into **Postgres** with `dpone`.

## When to use this path

Use this path when Kafka is the system of record or ingestion boundary and Postgres is the landing, warehouse, event-log, or downstream replication target.
...
defaults:
  name: kafka_to_postgres_example
  source:
    type: kafka
...
```bash
dpone plan examples/source-sink/kafka-to-postgres.yaml --format md
dpone run examples/source-sink/kafka-to-postgres.yaml
```

The checked source file is `examples/source-sink/kafka-to-postgres.yaml`; CI
...
4. Inspect the run artifact under `.dpone/runs/kafka_to_postgres`.
