...
Independent architecture and UX review identified unresolved admission details,
so this document remains DRAFT. Before approval, freeze a concrete ClickHouse
source-version provider and lifetime, enforceable server-capacity admission,
windowless full-refresh authoring, strategy-specific staged-port admission, and
the encoding worker execution model. The following algorithm is a proposal, not
permission to infer those capabilities. Track runtime patch removal separately in
the [removal inventory](runtime-patch-removal-inventory.md).
...
Journey: discover the route; verify source-version and target-transaction
...
transport. A runtime factory supplies source-version authority and independent
worker connector factories; credentials remain outside the manifest.
...
compatibility behavior; method presence alone must not admit them.
...
to the existing wire vocabulary, with route/type admission. Use existing
`execution.chunking.mode: bounded_window`, `parallelism`, and
`checkpointing: resumable` for logical window execution. Preserve their existing
meaning for other routes. Introduce no top-level transport registry.
...
| `max_stage_bytes` | required | Positive hard reservation for server staging |
...
Database staging and transaction-log requirements are separate capacity checks;
stop before exceeding the admitted stage budget. Driver allocation and RSS are
observed separately and are not guaranteed by encoded-byte limits.
...
Define `dpone.mssql_native_chunks.v1` receipts binding invocation, source version,
...
completion. Chunk identity never deduplicates business rows.
...
2. Acquire fenced invocation ownership. Obtain a source authority that identifies
   a verifiably immutable source version for all parallel logical reads. A query
   string, timestamp, row count or user assertion alone is not such authority.
   If unavailable, explicit resumable parallel window execution fails closed.
3. Persist the plan using existing durable CAS journal contracts. Derive logical
   chunks from fixed interval boundaries, never LIMIT/OFFSET over unordered rows.
4. Each logical worker owns one source iterator and one target connection. Pull
   typed rows under backpressure and validate range, arity, nullability and value
   bounds. Encode physical chunks directly to native binary files in the admitted
   work directory; never write a full-stream character intermediate.
...
6. Compare vendor count, reject status, server count and typed content evidence.
   ...
   A logical chunk becomes complete only after its source iterator reaches EOF
   and all physical attempts verify. Missing ordinals or duplicate receipts fail.
...
```text
admit -> fence -> immutable source version -> durable plan
  -> bounded native chunks -> isolated verified attempts -> complete logical chunks
...
after server identity, rows, digest and source version are reverified.

Allow at most two transient retries per logical chunk, persisted across restart.
...
If source-version authority expires before publication, require a new invocation
and complete re-extraction. Never resume a mutable source at a row offset. If all
logical chunks have durable EOF/verification evidence, publication recovery can
use those completed stages without re-reading source, subject to identity checks.
Published invocation recovery never reacquires source merely to complete evidence.
On retry of an incomplete logical chunk, fence and discard all its physical
attempts before reopening the query. A verified physical prefix cannot be reused
against a new unordered enumeration. Only a fully verified logical chunk with
durable EOF and unchanged source-version authority is eligible for reuse.
...
| ClickHouse source-version adapter | Immutable read authority and independent worker reads |
...
missing ordinals, expired source versions, cancellation and exhausted retries.
...
- [ ] Resolve the independent review admission gaps listed above.
