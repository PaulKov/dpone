...
- **PASS:** [CI for the frozen implementation](https://github.com/PaulKov/dpone/actions/runs/34467423669).
  This result applies to the named implementation commit, not subsequent integration.
...
The maintainer approved integration while preserving the parallel 0.75.0 release.
The release tag and source base are `67c8ade61732fb1bff267ce3bd26f2af776e79e6`.
All 91 upstream files outside the composed changelog, navigation and generated
metrics remain byte-identical to that source. Package versions and dependency
lock remain those of 0.75.0.

Combining the branches exceeded the runtime-to-contract dependency budget.
Pure transaction identity policy now resides in the canonical contracts package;
the runtime retains source resolution and injects route/backfill providers.
Public signatures, validation order, errors and identity hashes are preserved.
