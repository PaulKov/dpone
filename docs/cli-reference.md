...
- Usage: `dpone gitops airflow pack [--workload WORKLOAD] [--workload-set WORKLOAD_SET] [--env ENV] [--artifact-dir ARTIFACT_DIR] [--bundle-path BUNDLE_PATH] [--image IMAGE] [--image-digest IMAGE_DIGEST] [--mode {plan,verify}] [--runner-policy {advisory,pr,release}] [--outlet-binding {physical,logical}] [--include-live-gates] [--output-path OUTPUT_PATH] [--output OUTPUT] [--format {json,markdown}]`
- Options:
  ...
  - `--outlet-binding OUTLET_BINDING` — Bind Airflow outlets to deployment-resolved physical URIs or portable logical URIs
...
- Usage: `dpone gitops airflow reconcile --workload-set WORKLOAD_SET [--all-workloads] [--changed-files CHANGED_FILES [CHANGED_FILES ...]] [--changed-files-file CHANGED_FILES_FILE] [--env ENV] [--outlet-binding {physical,logical}] [--output-dir OUTPUT_DIR] [--output OUTPUT] [--format {json,markdown}]`
- Options:
...
- Usage: `dpone gitops airflow release-compose --manifest MANIFEST --output-dir OUTPUT_DIR [--format {json}]`
- Options:
  - `--manifest MANIFEST` — Local dpone.release-composition.v1 YAML; roots resolve relative to this file
  - `--output-dir OUTPUT_DIR` — Exact immutable destination outside both source roots; identical retries are verified
  - `--format FORMAT` — One JSON publication report; delivery does not authorize activation

### `dpone gitops airflow release-inventory`

- Usage: `dpone gitops airflow release-inventory --pack-root PACK_ROOT --xcom-sidecar-image XCOM_SIDECAR_IMAGE [--format {json}]`
- Options:
  - `--pack-root PACK_ROOT` — Complete ordinary reconcile root with _dags and producer workload packs
  - `--xcom-sidecar-image XCOM_SIDECAR_IMAGE` — Digest-pinned sidecar to validate supported strict transport
  - `--format FORMAT`
...
- Usage: `dpone airflow build --release-id RELEASE_ID --environment ENVIRONMENT [--trust-tier {production,non_production}] [--runtime-image-ref RUNTIME_IMAGE_REF] --runtime-image-digest RUNTIME_IMAGE_DIGEST [--runtime-image-dbt-ref RUNTIME_IMAGE_DBT_REF] [--runtime-image-dbt-digest RUNTIME_IMAGE_DBT_DIGEST] --artifact-registry-ref ARTIFACT_REGISTRY_REF [--registry-config-map-name REGISTRY_CONFIG_MAP_NAME] [--registry-config-map-key REGISTRY_CONFIG_MAP_KEY] [--registry-config-sha256 REGISTRY_CONFIG_SHA256] [--registry-credentials-secret-name REGISTRY_CREDENTIALS_SECRET_NAME] [--registry-credentials-secret-key REGISTRY_CREDENTIALS_SECRET_KEY] [--registry-credentials-connection-id REGISTRY_CREDENTIALS_CONNECTION_ID] [--registry-credentials-connection-type {airflow,env,vault}] [--registry-credentials-projection-mode {k8s_secret,env}] [--registry-credentials-env-name REGISTRY_CREDENTIALS_ENV_NAME] [--trust-policy-config-map-name TRUST_POLICY_CONFIG_MAP_NAME] [--trust-policy-config-map-key TRUST_POLICY_CONFIG_MAP_KEY] [--trust-policy-sha256 TRUST_POLICY_SHA256] [--airflow-bundle-ref AIRFLOW_BUNDLE_REF] [--dev-evidence-pvc-claim DEV_EVIDENCE_PVC_CLAIM] [--dev-evidence-worker-queue DEV_EVIDENCE_WORKER_QUEUE] [--runtime-authority-secret-name RUNTIME_AUTHORITY_SECRET_NAME] [--runtime-authority-secret-key RUNTIME_AUTHORITY_SECRET_KEY] [--runtime-authority-payload-file RUNTIME_AUTHORITY_PAYLOAD_FILE] [--runtime-authority-payload-sha256 RUNTIME_AUTHORITY_PAYLOAD_SHA256] [--format {text,json}]`
- Options:
  ...
  - `--registry-credentials-secret-name REGISTRY_CREDENTIALS_SECRET_NAME` — Kubernetes Secret containing one runtime artifact registry Airflow Connection
  - `--registry-credentials-secret-key REGISTRY_CREDENTIALS_SECRET_KEY` — Exact AIRFLOW_CONN_* key; derived from the connection id when omitted
  - `--registry-credentials-connection-id REGISTRY_CREDENTIALS_CONNECTION_ID` — Logical connection id used only by runtime init-fetch
  - `--registry-credentials-connection-type REGISTRY_CREDENTIALS_CONNECTION_TYPE` — Credential provider; follows the standard dpone connection_type vocabulary
  - `--registry-credentials-projection-mode REGISTRY_CREDENTIALS_PROJECTION_MODE` — How the selected credential variable is projected into the init container
  - `--registry-credentials-env-name REGISTRY_CREDENTIALS_ENV_NAME` — Exact environment variable exposed only to runtime init-fetch
  ...
  - `--runtime-authority-secret-name RUNTIME_AUTHORITY_SECRET_NAME` — Kubernetes Secret containing confidential runtime-authority configuration for a protected development release
  - `--runtime-authority-secret-key RUNTIME_AUTHORITY_SECRET_KEY` — Secret key to project read-only; defaults to authority.json when the Secret is selected
  - `--runtime-authority-payload-file RUNTIME_AUTHORITY_PAYLOAD_FILE` — Safe-to-persist non-secret authority payload file (1..4096 exact binary bytes); mutually exclusive with Secret mode
  - `--runtime-authority-payload-sha256 RUNTIME_AUTHORITY_PAYLOAD_SHA256` — Required canonical SHA-256 of exact payload file bytes; provides integrity, not confidentiality
  - `--format FORMAT`
