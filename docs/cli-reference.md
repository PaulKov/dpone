...
- Options:
  ...
  - `--composition-supervisor-pvc COMPOSITION_SUPERVISOR_PVC` — Administrator-provisioned ReadWriteMany PVC for v3 composition supervision
  - `--composition-child-uid-start COMPOSITION_CHILD_UID_START` — First reserved numeric child UID for v3 composition supervision
  - `--composition-child-gid-start COMPOSITION_CHILD_GID_START` — First reserved numeric child GID for v3 composition supervision
  - `--composition-child-identity-count COMPOSITION_CHILD_IDENTITY_COUNT` — Reserved child UID/GID range size; must be at least 1000000
  - `--format FORMAT`
...
- Usage: `dpone airflow cache-sync [--cache-root CACHE_ROOT] --deployment-dir DEPLOYMENT_DIR --environment ENVIRONMENT --promoted-by PROMOTED_BY --allowed-promoter ALLOWED_PROMOTER (--expected-current-deployment-id EXPECTED_CURRENT_DEPLOYMENT_ID | --expect-current-absent) [--source-commit SOURCE_COMMIT] [--attestation-ref ATTESTATION_REF] [--workspace-authority-connection-ref WORKSPACE_AUTHORITY_CONNECTION_REF] [--precommit-guard-path PRECOMMIT_GUARD_PATH] [--expected-precommit-guard-sha256 EXPECTED_PRECOMMIT_GUARD_SHA256] [--confirm-promote] [--format {text,json}]`
- Options:
  ...
  - `--workspace-authority-connection-ref WORKSPACE_AUTHORITY_CONNECTION_REF` — Protected runtime connection reference for native-v2 or complete-parent workspace admission; required by those release wires
