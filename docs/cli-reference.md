...
- Usage: `dpone init dbt --profiles DBT_PROFILES --profile DBT_PROFILE --workflow DBT_WORKFLOW [--dry-run] [--format {text,json,md}] PATH`
- Positional arguments:
  - `path` — Destination project; its parent directory must exist
- Options:
  - `--profiles DBT_PROFILES` — Explicit nonsecret platform policy file
  - `--profile DBT_PROFILE` — Native policy profile, not a recipe profile
  - `--workflow DBT_WORKFLOW` — Named workflow admitted by the policy
  - `--dry-run` — Plan without creating project files; default applies
  - `--format FORMAT`
- Output and exit contract:
  - `0`: bounded result on stdout; stderr is empty.
  - `1`: validation or conflict result on stdout; stderr is empty.
