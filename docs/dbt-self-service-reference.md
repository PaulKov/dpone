...
| Macro resolution | Generated exact 153-record dbt/dbt-sqlserver framework closure plus seven exact selected-node invocation records; identity, body digest, direct dependencies and dispatch families are pinned; selected nodes may otherwise call only the exact metadata-only `dpone_publish` helper |
...
Internally, `sqlserver_macro_authority_graph.py` owns the pure graph closure,
dispatch checks and macro records. The generator retains CLI/file handling,
artifact rendering and candidate reports, and preserves its previous imported
symbols as aliases. This separation changes neither authority membership nor
generated bytes; direct-script and module entry points use the same graph owner.
