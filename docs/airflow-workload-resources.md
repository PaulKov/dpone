...
Process-scoped `gitops` and `gitops.airflow` must be literal objects, as required
by the manifest schema. Dynamic/scalar containers are rejected before merge or
normalization can discard a declaration; for example, move
`defaults.gitops: "{{ airflow_options }}"` to the documented root configuration.
An override cannot cancel an unsupported declaration in defaults. Use literal
authoring containers (`processes`, `defaults`, `schemas` and
tables/overrides) so a whole-object template cannot conceal their structure.
