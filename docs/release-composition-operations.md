...
projection, cache installation, parse/selection checks, and successful artifact
verification are not activation approvals.
...
| Ownership or logical target collision | DAG IDs, workload IDs, paths, memberships, declared write coordinates, and exact XMin handoff metadata across both inputs | Correct authoring/publishing policy; for an intentional XMin handoff use exactly one `initial` and one `incremental` writer with the same public handoff contract, then rebuild both inventories; do not splice generated DAGs |
