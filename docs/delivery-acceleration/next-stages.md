...
| 2: independent stage limits | Minor; initial candidate 0.80.0, release pending | Separately configure encoding and import concurrency with bounded admission. | Written approved contract; old settings retain their behavior; deterministic queue/byte limits; cancellation, saturation, retry and recovery tests; narrow/wide local measurements and documented configuration. Automatic tuning is deferred. |
...
Evidence and release are pending. Narrow/wide measurements must retain each
