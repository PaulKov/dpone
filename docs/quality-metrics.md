...
🔴 **RED - the clustering hard gate fails on the current generation-admission candidate.**

General import/layer checks do not replace the independent architecture gates.
The current candidate exceeds the clustering limit in the canonical quality
budgets. General layer checks pass, but this candidate is not ready for merge.
The independent assertions in `tests/test_architecture_fitness_gate.py` remain
required. The generated
...
acceptance thresholds; both the hard clustering gate and preferred target remain unmet.
...
- Avg clustering: **0.184** → **WARN**
- Cross-layer ratio: **0.302** → **OK**
...
| `src/dpone/contracts/native_originals.py` | 423 |
...
- LCC ratio (largest connected component): **0.949**
- Avg clustering coefficient: **0.184**
- Cohesion ratio (share of deps within slice): **0.617**

**Heuristic interpretation**

- AVG Ce = **2.60** → **OK** (lower is usually better; big fan-out hints 'god modules')
- P90 Ce = **6** → **OK** (if P90 grows, responsibilities are spreading)
- Avg clustering = **0.184** → **WARN** (higher clustering often means more cycles/tight coupling)
...
- `dpone.contracts.airflow_deployment`: **93**
- `dpone.manifest.confined_files`: **78**
- `dpone.readiness.migration_control`: **76**
- `dpone.gitops.models`: **60**
- `dpone.contracts.dbt_contract_validation`: **58**
...
- Layers observed: **58**
- Internal import edges: **9659**
- Intra-layer edges: **6738**
- Cross-layer edges: **2921**
- Intra-layer ratio: **0.698**
- Cross-layer ratio: **0.302**

- Excluded layers: **`dpone.compat`** (150 modules, 139 edges removed from this coarse architecture view)

**Heuristic interpretation**

- Intra-layer ratio = **0.698** → **OK** (higher means more dependencies stay within their own layer)
- Cross-layer ratio = **0.302** → **OK** (lower means cleaner boundaries between layers)
...
| `dpone.adapters` | 199 | 483 | 285 | 192 | 291 | 93 | 2.43 | 1.43 | 0.398 | 5 | 5 |
...
| `dpone.app` | 25 | 205 | 38 | 20 | 185 | 18 | 8.20 | 1.52 | 0.098 | 10 | 4 |
| `dpone.backfill` | 73 | 171 | 207 | 139 | 32 | 68 | 2.34 | 2.84 | 0.813 | 6 | 5 |
...
| `dpone.config` | 16 | 33 | 117 | 18 | 15 | 99 | 2.06 | 7.31 | 0.545 | 3 | 10 |
| `dpone.connector_sdk` | 12 | 9 | 13 | 9 | 0 | 4 | 0.75 | 1.08 | 1.000 | 0 | 1 |
| `dpone.contracts` | 300 | 721 | 1583 | 709 | 12 | 874 | 2.40 | 5.28 | 0.983 | 6 | 18 |
| `dpone.core` | 7 | 8 | 1 | 1 | 7 | 0 | 1.14 | 0.14 | 0.125 | 4 | 0 |
| `dpone.dag` | 40 | 108 | 115 | 72 | 36 | 43 | 2.70 | 2.88 | 0.667 | 5 | 6 |
...
| `dpone.manifest` | 122 | 374 | 541 | 312 | 62 | 229 | 3.07 | 4.43 | 0.834 | 7 | 13 |
...
|---|---|---:|
| `dpone.runtime` | `dpone.contracts` | 229 |
| `dpone.adapters` | `dpone.contracts` | 159 |
| `dpone.services` | `dpone.contracts` | 159 |
| `dpone.adapters` | `dpone.ports` | 127 |
...
| `dpone.app` | `dpone.adapters` | 54 |
| `dpone.runtime` | `dpone.ports` | 54 |
| `dpone.ports` | `dpone.contracts` | 52 |
