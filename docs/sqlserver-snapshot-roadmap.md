...
| Dropped max_source_bytes — resolved | Compiler, public builder and staged-load regressions | Platform limit is preserved and checked before publication | Completed: aggregate source-emitted-byte admission | Complete measurement evidence | Exact limit/profile preserved through all handoffs; breach prevents publication/state advancement |
| Generated strategy vs public schema — resolved | Public strategy schema and compiler regression | Generated bounded strategy validates and reaches the enforcing runtime | Completed with runtime enforcement | Source-byte budget contract | Generated full-refresh manifest validates and reaches enforcing runtime or fails closed |
| Bounded ClickHouse full-refresh publication — resolved offline for local topology | Target-local marker, UUID classifier and fault-injection regressions | Admitted bounded loads use one reconciled EXCHANGE/RENAME and never blindly replay an unknown reply | Completed for local Atomic/Shared and non-replicated tables | Source-byte budget contract | Exact UUID mapping; unsupported topology rejected before mutation |
| One-shard replicated ClickHouse publication — researched, approval outstanding | [KeeperMap/distributed-DDL design](feature-design-clickhouse-cluster-full-refresh-publication-v1.md) | `ON CLUSTER` can finish on only part of the replica set; replaying EXCHANGE can restore old data on committed replicas | P1: strict one-shard/N-replica protocol with fail-closed terminal mixed outcome | Approved design, local bounded protocol, Docker fixture | One CAS dispatcher, exact queue entry, per-replica UUID/Keeper identity, zero blind replay or unproven cleanup |
| Legacy unbounded ClickHouse full-refresh atomicity — confirmed compatibility debt | Existing multi-rename fallback and official RENAME semantics in research register | Unbounded manifests retain non-atomic compatibility behavior | P1: deliberate migration to the bounded publication capability | Migration/deprecation decision | No silent behavior change; migrated routes satisfy the bounded publication contract |
...
| Budget docs incomplete — resolved | dbt self-service and MSSQL to ClickHouse runbooks | Users can identify metric, failure behavior and recovery boundary | Completed with budget enforcement | Source-byte budget contract | First-success plus exceeded/unknown examples, exact units and limitations |
...
| 2 | Explicit full-refresh safety fail-closed bridge — complete | No explicit budget disappears or claims enforcement | Superseded by the enforcing runtime contract |
| 3 | Versioned byte metric and end-to-end enforcement — complete | Schema/compile/YAML/pack/runtime/evidence agree; boundary and no-publish tests pass | Aggregate source-emitted-byte metric is enforced before publication |
| 4 | ClickHouse atomic publication and recovery — bounded local path complete | Admitted local engine/topology, target-local identity marker, no double exchange, exact cleanup | Live certification and legacy unbounded migration remain separate |
| 5 | One-shard/N-replica ClickHouse publication — design researched | KeeperMap CAS, exact distributed-DDL identity, per-replica reconciliation, terminal mixed fail-closed | Architecture approval precedes runtime work; no blind `ON CLUSTER` replay |
| 6 | Catalog truth and bounded cleanup gaps | Explicit unsupported observation; exact owned-object cleanup only where audit proves missing protection | Separate introspection and cleanup PRs; reuse existing protections |
| 7 | dbt ordinary CCI table admission | Opt-in canonical mapping to pinned materialization, pre-publish proof, hooks closed | Safety and macro-authority review; no incremental expansion |
| 8 | dbt ROW/PAGE governed bridge | Same canonical semantics, verified intermediate DDL, no uncontrolled hooks | Separate macro/authority design approval |
| 9 | Synthetic benchmark harness and scoped certification | Exact values, recovery and measured resource/throughput/scan evidence | Harness can be developed early; live execution requires approved stand |
| 10 | Advanced physical tuning | Archive, NCCI, constraints, partitions, incremental migrations in separate specs | Only selected evidence-backed scope; no automatic winner/default |
