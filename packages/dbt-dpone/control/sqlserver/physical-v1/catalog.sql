...
SELECT @object_name=t.name FROM sys.tables t WHERE t.object_id=@object_id
...
DECLARE @row_count_exact bigint,@count_sql nvarchar(max)=N'SELECT @count=COUNT_BIG(*) FROM '
...
IF @row_count_exact IS NULL OR @row_count_exact<0 OR NOT EXISTS
...
IF EXISTS (SELECT 1 FROM sys.indexes WHERE object_id=@object_id
