...
def evaluate_clickhouse_cluster_admission(
    ...
    blockers: list[str] = []
    if not _REPLICATED_MERGE_TREE.match(request.engine.strip()):
    ...
    return ClickHouseClusterAdmissionDecision(
        ...
        mode="cluster" if not blockers else "blocked",
...
def clickhouse_cluster_admission_input(
    *,
    ...
    cluster_name, ddl_scope = _cluster(clickhouse.get("cluster"))
...
def _cluster(raw: object) -> tuple[str | None, str]:
    if isinstance(raw, str):
        name = raw.strip() or None
        return name, "cluster" if name else "local"
    values = _mapping(raw)
    name = str(values.get("name") or "").strip() or None
    if "ddl_scope" in values:
        return name, str(values.get("ddl_scope") or "local").strip().lower()
    return name, "cluster" if bool(values.get("on_cluster", False)) else "local"
