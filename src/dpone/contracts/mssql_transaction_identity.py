...
def invocation_identity(
    run_context: Any, load_config: Any, *, dag_id: str | None, execution_policy_resolver: Callable[[Any], Any]
) -> InvocationIdentity:
    """Resolve scheduler-stable run/process/task identity without using load_id."""

    run_id = (
        _backfill_campaign_invocation_id(load_config, execution_policy_resolver)
        or str(getattr(run_context, "run_id", "") or "").strip()
    )
    if not run_id:
...
def _backfill_campaign_invocation_id(load_config: Any, execution_policy_resolver: Callable[[Any], Any]) -> str | None:
    ...
    if context is None:
        return None
    try:
        execution_policy_resolver(load_config)
...
def build_mssql_attempt_request(
    ...
    *,
    ...
    route_fingerprint: Callable[..., bytes],
) -> MssqlAttemptRequest:
...
def operation_request(
    load_config: Any, invocation: InvocationIdentity, *, backfill_policy_digest: Callable[[Any], str | None]
) -> MssqlOperationRequest:
    ...
    if raw is None:
        ...
        if backfill_policy_digest(load_config) is None:
...
def validate_source_physical_identity(
    resolved: Any, load_config: Any, *, expected: str, expected_digest: str | None
) -> SourcePhysicalIdentity:
    """Validate an already resolved source binding without connector I/O."""
