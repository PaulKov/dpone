...
def _dimensions(dtype: str) -> tuple[str, int, int, int, int]:
    ...
    if base in {"time", "datetime2", "datetimeoffset"}:
        scale = int(values[0])
        type_id, fixed_bytes, precision = {"time": (41, 0, 8), "datetime2": (42, 3, 19), "datetimeoffset": (43, 5, 26)}[
            base
        ]
        # sys.columns reports scale-dependent rowstore storage, not the
        # fixed-width in-memory or serialized time representation.
        length = fixed_bytes + (3 if scale <= 2 else 4 if scale <= 4 else 5)
