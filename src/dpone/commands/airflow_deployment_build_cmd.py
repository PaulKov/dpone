...
from dpone.readiness.airflow_deployment_errors import (
    deployment_projection_error,
    deployment_projection_exit_code,
)
...
def cmd_airflow_build(args: argparse.Namespace, *, ctx: object, logger: logging.Logger) -> int:
    ...
    if supervisor_error is not None:
        ...
        result = _build_supervised_deployment_result(args, supervisor)
    else:
        result = _build_legacy_deployment_result(args)
...
def _build_legacy_deployment_result(args: argparse.Namespace) -> SelfServiceResult:
...
def _build_supervised_deployment_result(
    ...
    trust_policy_ref = _trust_policy_ref(args)
    if (
        args.trust_tier is None
        or args.runtime_image_ref is None
        or not _complete_config_map_ref(registry_config_ref)
        or (args.trust_tier == "production" and not _complete_config_map_ref(trust_policy_ref))
    ):
        return _build_legacy_deployment_result(args)
    try:
        projection = AirflowDeploymentProjectionService(root=".").materialize(
            release_id=args.release_id,
            environment=args.environment,
            trust_tier=args.trust_tier,
            runtime_image_ref=args.runtime_image_ref,
            ...
            dev_evidence_pvc_claim=args.dev_evidence_pvc_claim,
            dev_evidence_worker_queue=args.dev_evidence_worker_queue,
            composition_supervisor=supervisor.to_dict(),
        )
    ...
    details["deployment_dir"] = projection.deployment_dir.resolve(strict=False).relative_to(root).as_posix()
...
def _complete_config_map_ref(value: dict[str, object] | None) -> bool:
    return value is not None and all(
        isinstance(value.get(field), str) and bool(str(value[field]).strip()) for field in ("name", "key", "sha256")
    )
