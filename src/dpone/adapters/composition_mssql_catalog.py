...
def require_composition_catalog_visibility(cursor: SqlControlCursor, schema: str) -> None:
    ...
    _require(
        ...
        "CASE WHEN EXISTS (SELECT 1 FROM sys.database_permissions p JOIN sys.user_token u "
        "ON u.principal_id=p.grantee_principal_id WHERE p.state='D' AND p.permission_name IN "
        "('CONTROL','VIEW DEFINITION','VIEW SECURITY DEFINITION','VIEW PERFORMANCE DEFINITION')) "
        "THEN 1 ELSE 0 END FROM sys.schemas s WHERE s.name=?;",
        ((schema, 1, 1, 0),),
        schema,
    )
...
def inspect_composition_table(
    ...
    _require(
        cursor,
        "table",
        "SCHEMA_NAME(t.schema_id),t.name,CONVERT(int,t.uses_ansi_nulls),"
        "HAS_PERMS_BY_NAME(QUOTENAME(SCHEMA_NAME(t.schema_id))+N'.'+QUOTENAME(t.name),N'OBJECT',N'VIEW DEFINITION'),"
        "t.temporal_type,CONVERT(int,t.is_memory_optimized),CONVERT(int,t.is_filetable),"
        "CONVERT(int,t.is_external),CONVERT(int,t.is_node),CONVERT(int,t.is_edge),t.ledger_type,"
        "CONVERT(int,t.is_dropped_ledger_table),t.durability,CONVERT(int,t.is_remote_data_archive_enabled),"
        "CONVERT(int,t.is_replicated),CONVERT(int,t.has_replication_filter),CONVERT(int,t.is_merge_published),"
        "CONVERT(int,t.is_sync_tran_subscribed),CONVERT(int,t.is_tracked_by_cdc),"
        "CASE WHEN EXISTS (SELECT 1 FROM sys.fulltext_indexes f WHERE f.object_id=t.object_id) THEN 1 ELSE 0 END,"
        "CASE WHEN EXISTS (SELECT 1 FROM sys.change_tracking_tables c WHERE c.object_id=t.object_id) THEN 1 ELSE 0 END,"
        "t.history_table_id,t.ledger_view_id FROM sys.tables t WHERE t.object_id=OBJECT_ID(?,N'U');",
    ...
    _require(
        cursor,
        "columns",
        "c.column_id,c.name,TYPE_NAME(c.system_type_id),c.max_length,c.collation_name,CONVERT(int,c.is_nullable),"
        "CASE WHEN c.user_type_id=c.system_type_id THEN 1 ELSE 0 END,CONVERT(int,c.is_identity),"
        "CONVERT(int,c.is_computed),c.default_object_id,c.rule_object_id,c.generated_always_type,"
        "CONVERT(int,c.is_hidden),c.encryption_type,"
        "CASE WHEN EXISTS (SELECT 1 FROM sys.masked_columns m "
        "WHERE m.object_id=c.object_id AND m.column_id=c.column_id AND m.is_masked=1) THEN 1 ELSE 0 END,"
        "CONVERT(int,c.is_sparse),CONVERT(int,c.is_column_set),CONVERT(int,c.is_filestream),"
        "CONVERT(int,c.is_rowguidcol),CONVERT(int,c.is_ansi_padded) "
...
def _keys(cursor: SqlControlCursor, name: str, table: CompositionTable, order: str) -> None:
    ...
    _require(
        cursor,
        "keys",
        "i.name,c.name,ic.key_ordinal,ic.index_column_id,CONVERT(int,ic.is_descending_key),"
        "CONVERT(int,ic.is_included_column),CONVERT(int,i.is_primary_key),CONVERT(int,i.is_unique),"
        "CONVERT(int,i.is_unique_constraint),i.type,CONVERT(int,i.is_disabled),CONVERT(int,i.has_filter),"
        "CONVERT(int,i.ignore_dup_key),DATALENGTH(i.filter_definition),CONVERT(int,i.is_hypothetical),kc.name,kc.type_desc "
        ...
        f"WHERE i.object_id=OBJECT_ID(?,N'U') ORDER BY i.name{order},ic.index_column_id;",
        keys,
        name,
    )
...
def _constraints(
    ...
    _require(
        cursor,
        "foreign_keys",
        "f.name,SCHEMA_NAME(f.schema_id),k.constraint_column_id,pc.name,OBJECT_SCHEMA_NAME(f.referenced_object_id),"
        "rt.name,rc.name,CONVERT(int,f.is_disabled),CONVERT(int,f.is_not_trusted),f.delete_referential_action,"
        "f.update_referential_action,CONVERT(int,f.is_not_for_replication),CONVERT(int,f.is_system_named) "
        "FROM sys.foreign_keys f LEFT JOIN sys.foreign_key_columns k ON k.constraint_object_id=f.object_id "
        "LEFT JOIN sys.columns pc ON pc.object_id=k.parent_object_id AND pc.column_id=k.parent_column_id "
        "LEFT JOIN sys.tables rt ON rt.object_id=f.referenced_object_id "
        "LEFT JOIN sys.columns rc ON rc.object_id=k.referenced_object_id AND rc.column_id=k.referenced_column_id "
        f"WHERE f.parent_object_id=OBJECT_ID(?,N'U') ORDER BY f.name{order},k.constraint_column_id;",
        foreign,
        name,
    )
    expected: Rows = tuple(
    ...
    _require(
        cursor,
        "checks",
        "c.name,SCHEMA_NAME(c.schema_id),c.parent_column_id,"
        "CASE WHEN DATALENGTH(c.definition)<=? THEN c.definition END,DATALENGTH(c.definition),"
        "CONVERT(int,c.is_disabled),CONVERT(int,c.is_not_trusted),CONVERT(int,c.is_not_for_replication),"
        "CONVERT(int,c.uses_database_collation),CONVERT(int,c.is_system_named) "
        f"FROM sys.check_constraints c WHERE c.parent_object_id=OBJECT_ID(?,N'U') ORDER BY c.name{order};",
        expected,
        maximum,
        name,
    )
    ...
    _require(
        cursor,
        "objects",
        "o.name,o.type_desc,SCHEMA_NAME(o.schema_id),CONVERT(int,o.is_ms_shipped) "
        f"FROM sys.objects o WHERE o.parent_object_id=OBJECT_ID(?,N'U') ORDER BY o.name{order};",
        expected,
        name,
    )
...
def _modules(cursor: SqlControlCursor, schema: str, name: str, module: CompositionTrigger, order: str) -> None:
    ...
    _require(
        cursor,
        "triggers",
        "t.name,OBJECT_SCHEMA_NAME(t.object_id),t.type_desc,t.parent_class,CONVERT(int,t.is_ms_shipped),"
        "CONVERT(int,t.is_disabled),CONVERT(int,t.is_instead_of_trigger),CONVERT(int,t.is_not_for_replication),"
        "CONVERT(int,m.uses_ansi_nulls),CONVERT(int,m.uses_quoted_identifier),m.execute_as_principal_id,"
        "CONVERT(int,m.is_schema_bound),CONVERT(int,m.uses_native_compilation),"
        "HASHBYTES('SHA2_256',m.definition),DATALENGTH(m.definition) "
        "FROM sys.triggers t LEFT JOIN sys.sql_modules m ON m.object_id=t.object_id "
        f"WHERE t.parent_id=OBJECT_ID(?,N'U') ORDER BY t.name{order};",
    ...
    _require(
        cursor,
        "events",
        "t.name,e.type_desc,CONVERT(int,e.is_first),CONVERT(int,e.is_last) "
        "FROM sys.trigger_events e JOIN sys.triggers t ON t.object_id=e.object_id "
        f"WHERE t.parent_id=OBJECT_ID(?,N'U') ORDER BY t.name{order},e.type_desc{order};",
