...
class YandexWebmasterReplaceExtractStrategy(APIBaseStrategy):
    ...
    def extract(self, load_config: Any, last_state: dict[str, Any] | None) -> ExtractResult:
        ...
        default_days = int(
            options.get("lookback_days", spec.default_lookback_days or 14) or (spec.default_lookback_days or 14)
        )
        from_day, to_day = resolve_window(options=options, default_days=default_days)
        load_config.custom_predicate = build_date_replace_predicate(date_from=from_day, date_to=to_day)

        self.logger.log_etl_progress(
            "YANDEX_WEBMASTER_REPLACE_EXTRACT",
            {
                "API": "yandex_webmaster",
                "Resource": resource,
                "From_Day": from_day.isoformat(),
                "To_Day": to_day.isoformat(),
                "Predicate": load_config.custom_predicate,
            },
        )
