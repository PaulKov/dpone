...
class CompositionPackExecutionDispatcher:
    ...
    def __init__(
        ...
        *,
        ...
        native_executor_factory: Callable[[], Any | None] | None = None,
    ) -> None:
        self._native = CompositionNativeDbtDispatcher(native_executor, supervisor=supervisor)
        self._native_executor_factory = native_executor_factory
    ...
    def run(self, request: CompositionDispatchRequest) -> int:
        if request.kind == "native_dbt":
            if self._native_executor_factory is not None:
                native = CompositionNativeDbtDispatcher(self._native_executor_factory(), supervisor=self._supervisor)
    ...
    def _run_ordinary(self, request: CompositionDispatchRequest) -> int:
        ...
        try:
            ...
            raise CompositionDispatchRejection(ORDINARY_WORKER_UNAVAILABLE) from None
        from dpone.app.composition_remote_clickhouse_worker import RemoteClickHouseNonSuccess
        ...
        try:
            ...
            _write_pack_evidence(
                ...
                cell,
                {
                    "status": error.response.status.lower(),
                    "remote_response_document": strict_json_object(error.response_document),
                },
            )
