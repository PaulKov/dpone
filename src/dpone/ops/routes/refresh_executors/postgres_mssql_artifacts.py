"""Artifact rendering for Postgres -> MSSQL refresh chunks."""
...
from .native_pipeline import NativeChunkExportResult, NativeChunkLoadResult, NativeChunkPrepareResult
from .postgres_mssql_config import PostgresMssqlRefreshConfig

CHUNK_SCHEMA_VERSION = "dpone.postgres_mssql.route_refresh_chunk.v1"


def chunk_artifact_path(request: RouteRefreshChunkExecutionRequest) -> Path:
    return Path(request.output_dir) / "chunks" / f"postgres_mssql_refresh_chunk_{request.ordinal:04d}.json"


def transfer_path(request: RouteRefreshChunkExecutionRequest) -> Path:
    return Path(request.output_dir) / "chunks" / f"postgres_mssql_refresh_chunk_{request.ordinal:04d}.bcp"
...
def write_success_artifact(
    *,
    ...
    query: str,
    transfer_file: Path,
    export_result: NativeChunkExportResult,
    prepare_result: NativeChunkPrepareResult,
    load_result: NativeChunkLoadResult,
) -> Path:
