"""Route live certification policy."""
...
class RouteLiveCertificationDecision:
    """Pure go/no-go decision for route live certification evidence."""
...
class RouteLiveCertificationPolicy:
    """Evaluate live route evidence without knowing route-specific mechanics."""

    def evaluate(
        self,
        *,
        required_evidence: tuple[str, ...],
        evidence: Sequence[_LiveEvidence],
        ...
        return RouteLiveCertificationDecision(
...
def _score(*, required_evidence: tuple[str, ...], evidence: Sequence[_LiveEvidence]) -> float:
...
def _next_actions(blockers: tuple[str, ...]) -> tuple[str, ...]:
    ...
    for blocker in blockers:
        ...
        if blocker.startswith("route.unsupported"):
            actions.append("Add the route to the source -> sink matrix before live certification.")
        elif suffix == "missing":
            actions.append(f"Run the upstream live check and attach `{name}` evidence.")
