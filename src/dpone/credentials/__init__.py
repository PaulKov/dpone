...
Use `dpone.runtime.credentials` instead of `dpone.credentials`.
...
_warnings.warn(
    "`dpone.credentials` is deprecated; use `dpone.runtime.credentials`",
    DeprecationWarning,
    stacklevel=2,
)
...
def _runtime() -> Any:
    global _RUNTIME
    if _RUNTIME is None:
        _RUNTIME = import_module("dpone.runtime.credentials")
    return _RUNTIME
