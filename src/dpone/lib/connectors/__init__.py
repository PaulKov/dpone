...
Use `dpone.runtime.connectors` instead of `dpone.lib.connectors`.
...
_warnings.warn(
    "`dpone.lib.connectors` is deprecated; use `dpone.runtime.connectors`",
    DeprecationWarning,
    stacklevel=2,
)
...
def _runtime() -> Any:
    global _RUNTIME
    if _RUNTIME is None:
        _RUNTIME = import_module("dpone.runtime.connectors")
    return _RUNTIME
