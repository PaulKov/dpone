"""File-IO facade for schema migration release watch commands."""
...
class MigrationWatchFacade:
    """Application facade for watch plan/run/certify/report artifacts."""

    def plan(
        self,
        *,
        pack_path: str,
        post_apply_certificate_path: str,
        manifest_path: str,
        ...
        return MigrationWatchPlanner().plan(
            pack=_read_mapping(pack_path),
            post_apply_certificate=_read_mapping(post_apply_certificate_path),
    ...
    def run(self, *, plan_path: str, execute: bool = False) -> dict[str, Any]:
        ...
        try:
            return MigrationWatchRunner().run(plan=plan, probe=adapter, execute=execute)
        finally:
            close = getattr(adapter, "close", None)
            if callable(close):
                close()

    def status(self, *, run_path: str) -> dict[str, Any]:
        return _read_mapping(run_path)

    def certify(self, *, run_path: str, profile: str = "stage") -> dict[str, Any]:
        return MigrationWatchCertifier().certify(run=_read_mapping(run_path), profile=profile)

    def report(self, *, certificate_path: str) -> dict[str, Any]:
        certificate = _read_mapping(certificate_path)
        renderer = import_module("dpone.readiness.schema_migration_watch_rendering")
        return {**certificate, "markdown": renderer.render_watch_markdown(certificate)}
...
def _load_adapter(plan: dict[str, Any]) -> Any:
    ...
    if sink_type == "clickhouse":
        module = import_module("dpone.runtime.sinks.clickhouse_watch")
        return module.ClickHouseWatchProbe.from_config(connection)
    raise ValueError(f"release watch is implemented for clickhouse in v1, got {sink_type!r}")
...
def _connection_path(plan: dict[str, Any]) -> str:
    ...
    raise ValueError("watch plan does not contain target_connection.path")
