...
@pytest.mark.parametrize("dtype", ["Decimal(9,2)", "Decimal32(2)"])
@pytest.mark.parametrize("value,code", [("10000000", "out_of_range"), ("1.001", "precision_loss")])
def test_bounded_decimal_preserves_published_error_codes(dtype, value, code):
    with pytest.raises(ValueError, match=f"^clickhouse_binary_decimal_{code}"):
        list(encoder(dtype, max_batch_bytes=4).iter_batches([(value,)]))


@pytest.mark.parametrize("dtype", ["Date32", "DateTime64(6)"])
@pytest.mark.parametrize("year", [1899, 2300])
def test_bounded_temporal_rejects_server_calendar_clamping(dtype, year):
    from datetime import UTC, datetime

    with pytest.raises(ValueError, match="^clickhouse_binary_temporal_out_of_range$"):
        list(encoder(dtype, max_batch_bytes=8).iter_batches([(datetime(year, 1, 1, tzinfo=UTC),)]))
