...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("time(0)", "time", 41, 3, 8, 0, None, False),
        ("datetimeoffset(3)", "datetimeoffset", 43, 9, 30, 3, None, False),
    ],
)
...
@pytest.mark.parametrize(
    "type_name,type_id,lengths,precisions",
    [
        ("time", 41, (3, 3, 3, 4, 4, 5, 5, 5), (8, 10, 11, 12, 13, 14, 15, 16)),
        ("datetime2", 42, (6, 6, 6, 7, 7, 8, 8, 8), (19, 21, 22, 23, 24, 25, 26, 27)),
        ("datetimeoffset", 43, (8, 8, 8, 9, 9, 10, 10, 10), (26, 28, 29, 30, 31, 32, 33, 34)),
    ],
)
@pytest.mark.parametrize("scale", range(8))
def test_live_observed_temporal_dimensions(type_name, type_id, lengths, precisions, scale):
    # Exact SQL2022 vectors retained in catalog-live-09740e5.xml; not computed
    # using the implementation under test. The shared assertion rejects drift.
    test_physical_dimensions(
        f"{type_name}({scale})", type_name, type_id, lengths[scale], precisions[scale], scale, None, False
    )
