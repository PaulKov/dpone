...
from tests.test_mssql_tds_coordinator_evidence import OUTERS, H


def record(kind=Kind.ADMISSION):
    return TdsCoordinatorEvidenceRecord(H, kind, canonical_json_bytes(OUTERS[kind]))


def opened(pool, factory, deadline=None):
    return pool.open(
        lambda d, clock: TdsCoordinatorEvidenceActor(factory, H, d, clock),
...
def test_real_filesystem_writer_construction_write_and_teardown_on_actor(tmp_path):
    ...
    assert gateway.observation == TdsCoordinatorEvidenceObservation(H, receipt)
...
def test_each_kind_is_one_shot_and_only_six_backend_writes():
    writes = []
    ...
    try:
        for kind in Kind:
            ...
            with pytest.raises(ValueError, match="consumed"):
                gateway.write(record(kind), deadline=monotonic() + 1)
        assert len(writes) == len(set(writes)) == 6
    finally:
...
def test_wrong_operation_or_forged_shape_cannot_reach_backend():
    ...
    try:
        with pytest.raises(ValueError, match="operation_invalid"):
            gateway.write(replace(record(), operation_sha256="b" * 64), deadline=monotonic() + 1)
        forged = record()
...
def test_corrupt_ack_is_rejected_without_exposing_new_observation(monkeypatch):
    ...
    monkeypatch.setattr(gateway, "_dispatch", lambda *args: TdsCoordinatorEvidenceObservation(H, wrong))
    try:
        with pytest.raises(TdsJournalActorUnknown):
            gateway.write(record(), deadline=monotonic() + 1)
...
@pytest.mark.parametrize("value", [None, True, 1, "", "A" * 64, "a" * 63, "a" * 65])
def test_constructor_validation_is_exception_equivalent_to_observation(value):
    with pytest.raises((TypeError, ValueError)) as expected:
        TdsCoordinatorEvidenceObservation(value)
    with pytest.raises(type(expected.value), match=f"^{str(expected.value)}$"):
        TdsCoordinatorEvidenceActor(sink, value, monotonic() + 1, monotonic)


def test_constructor_accepts_valid_lowercase_sha():
    pool = TdsActorPool(capacity=1)
    gateway = opened(pool, sink)
    gateway.close(deadline=monotonic() + 1)
