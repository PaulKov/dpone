...
def _artifact_args(tmp_path: Path, route: RouteKey, *, passed: bool = True) -> list[str]:
    ...
    for name in names:
        ...
        path = _write_json(tmp_path / f"{name}.json", payload)
        values.extend(["--artifact", f"{name}={path}"])
    return values


def test_ops_route_release_gate_cli_outputs_json_and_writes_receipt(
...
def test_ops_route_release_gate_cli_returns_nonzero_for_failed_required_artifact(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
    tmp_path: Path,
) -> None:
