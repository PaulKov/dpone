    ...
    for name in names:
        path = _write_json(
            tmp_path / f"{name}.json",
            {
                ...
                "production_certification": "VERIFIED" if passed else "UNVERIFIED",
...
def test_ops_route_live_certification_cli_outputs_json_bundle(
    ...
    assert payload["evidence_index"]["cdc_apply_certification"]["passed"] is True
    assert Path(payload["json_path"]).exists()
    assert Path(payload["markdown_path"]).exists()


def test_ops_route_live_certification_cli_returns_nonzero_for_failed_required_artifact(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
    tmp_path: Path,
) -> None:
    ...
    assert "route_readiness.not_passed" in payload["blockers"]
