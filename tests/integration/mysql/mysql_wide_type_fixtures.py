"""Wide MySQL fixtures for pair-profile live certification.

Covers round-tripable types from `mysql_to_bigquery_analytics_v1`.
Contract-required types (ENUM/SET/spatial/BIGINT UNSIGNED/YEAR/overflow DECIMAL)
are intentionally omitted from the live wide table.
"""
...
WIDE_TABLE = "mysql_to_bigquery_wide"
...
class WideColumn:
    ...
    mysql_type: str
    seed_sql: str
    expected_bq_type: str


def wide_columns() -> list[WideColumn]:
    """Identity + business + type-profile columns for CSV→BigQuery round trip."""
    ...
    specs: list[tuple[str, str, str, str]] = [
        ...
        # Blob exports as hex over CSV; assert schema presence, not byte equality.
        ("c_blob", "BLOB NULL", "blob", "X'010203'"),
        ("c_name", "VARCHAR(64) NOT NULL", "varchar(64)", "'row-one'"),
    ]
    columns: list[WideColumn] = []
    for name, ddl, mysql_type, seed in specs:
        decision = mapper.resolve(mysql_type)
        columns.append(
            WideColumn(
                name=name,
                mysql_ddl=f"`{name}` {ddl}",
                mysql_type=mysql_type,
                seed_sql=seed,
                expected_bq_type=decision.target_type,
            )
        )
    return columns
...
def create_wide_mysql_table(mysql, *, table: str = WIDE_TABLE) -> list[WideColumn]:
    ...
    # Second row for merge/diff cardinality checks (watermark later).
    values2 = []
    for col in columns:
        if col.name == "id":
            values2.append("2")
