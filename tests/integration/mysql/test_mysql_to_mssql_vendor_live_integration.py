"""Docker MySQL → MSSQL vendor-live: wide types + all MSSQL load strategies."""
...
from tests.integration.mysql import mysql_mssql_strategy_configs as cfg
...
from tests.integration.mysql.mysql_mssql_assertions import (
    assert_row_count,
    assert_typed_spot_checks,
    assert_unique_ids,
    fq,
)
from tests.integration.mysql.mysql_mssql_wide_fixtures import (
    WIDE_TABLE,
...
@pytest.mark.skipif(not mysql_mssql_enabled(), reason="MySQL/MSSQL Docker IT not configured")
def test_mysql_to_mssql_full_refresh_wide_live(tmp_path: Path) -> None:
    table = f"{WIDE_TABLE}_fr"
    mysql, mssql, columns = _ready(table)
    config = cfg.full_refresh(mysql_database=mysql.database, tmp_path=tmp_path, table=table)
    extract = MySQLFullExtractStrategy(mysql, logger=NoopLogger(), sink_connector=mssql).extract(config, None)
    assert getattr(extract.artifact, "format", None) == "mssql-delimited"
    assert getattr(extract.artifact, "rows_exported", None) == 2
    result = _load(MSSQLSink(mssql, logger=NoopLogger()), config, extract.artifact, extract.schema)
    assert result.total_rows == 2
    assert_row_count(mssql, table=table, expected=2)
    assert_typed_spot_checks(mssql, table=table, columns=columns)


@pytest.mark.skipif(not mysql_mssql_enabled(), reason="MySQL/MSSQL Docker IT not configured")
def test_mysql_to_mssql_incremental_append_wide_live(tmp_path: Path) -> None:
    table = f"{WIDE_TABLE}_append"
    mysql, mssql, columns = _ready(table)
    config = cfg.incremental_append(mysql_database=mysql.database, tmp_path=tmp_path, table=table)
    strategy = MySQLIncrementalExtractStrategy(mysql, logger=NoopLogger(), sink_connector=mssql)
    ...
    assert_row_count(mssql, table=table, expected=3)
    assert_typed_spot_checks(mssql, table=table, columns=columns)


@pytest.mark.skipif(not mysql_mssql_enabled(), reason="MySQL/MSSQL Docker IT not configured")
def test_mysql_to_mssql_incremental_merge_wide_live(tmp_path: Path) -> None:
    ...
    assert_row_count(mssql, table=table, expected=2)
    row = mssql.get_records(f"SELECT c_name FROM {fq(table)} WHERE id = 1", as_dict=True)[0]
    ...
    _load(sink, config, third.artifact, third.schema)
    assert_row_count(mssql, table=table, expected=3)
    assert_typed_spot_checks(mssql, table=table, columns=columns, expected_name="merged")


@pytest.mark.skipif(not mysql_mssql_enabled(), reason="MySQL/MSSQL Docker IT not configured")
def test_mysql_to_mssql_replace_wide_live(tmp_path: Path) -> None:
    table = f"{WIDE_TABLE}_replace"
    mysql, mssql, columns = _ready(table)
    fr = cfg.full_refresh(mysql_database=mysql.database, tmp_path=tmp_path, table=table)
    extract = MySQLFullExtractStrategy(mysql, logger=NoopLogger(), sink_connector=mssql).extract(fr, None)
    sink = MSSQLSink(mssql, logger=NoopLogger())
    ...
    replaced = MySQLFullExtractStrategy(mysql, logger=NoopLogger(), sink_connector=mssql).extract(config, None)
    ...
    assert_typed_spot_checks(mssql, table=table, columns=columns, expected_name="replaced")


@pytest.mark.skipif(not mysql_mssql_enabled(), reason="MySQL/MSSQL Docker IT not configured")
def test_mysql_to_mssql_partition_replace_wide_live(tmp_path: Path) -> None:
    ...
    batch = MySQLFullExtractStrategy(mysql, logger=NoopLogger(), sink_connector=mssql).extract(config, None)
    ...
    assert_unique_ids(mssql, table=table)
    assert_typed_spot_checks(mssql, table=table, columns=columns, expected_name="part-replaced")


@pytest.mark.skipif(not mysql_mssql_enabled(), reason="MySQL/MSSQL Docker IT not configured")
def test_mysql_to_mssql_snapshot_diff_wide_live(tmp_path: Path) -> None:
    table = f"{WIDE_TABLE}_diff"
    mysql, mssql, columns = _ready(table)
    config = cfg.snapshot_diff(mysql_database=mysql.database, tmp_path=tmp_path, table=table)
    strategy = MySQLFullExtractStrategy(mysql, logger=NoopLogger(), sink_connector=mssql)
    sink = MSSQLSink(mssql, logger=NoopLogger())
    ...
    assert_typed_spot_checks(mssql, table=table, columns=columns, expected_name="diff-updated")


@pytest.mark.skipif(not mysql_mssql_enabled(), reason="MySQL/MSSQL Docker IT not configured")
def test_mysql_to_mssql_scd2_wide_live(tmp_path: Path) -> None:
    ...
    assert_typed_spot_checks(mssql, table=table, columns=columns, expected_name="scd2-v2", check_seed_scalars=False)


@pytest.mark.skipif(not mysql_mssql_enabled(), reason="MySQL/MSSQL Docker IT not configured")
def test_mysql_to_mssql_backfill_replace_wide_live(tmp_path: Path) -> None:
    ...
    assert_typed_spot_checks(mssql, table=table, columns=columns, expected_name="backfilled")
