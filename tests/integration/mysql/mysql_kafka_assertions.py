"""Assertions for mysql→Kafka wide live certification (JSON message payloads)."""
...
def assert_message_count(envelopes: Sequence[Mapping[str, Any]], *, expected: int) -> None:
    assert len(envelopes) == expected, f"expected {expected} messages, got {len(envelopes)}"
...
def assert_typed_spot_checks(
    ...
    *,
    ...
    assert str(row["c_bool"]) in {"1", "true", "True"}
    assert Decimal(str(row["c_decimal"])) == Decimal("12.3456")
    assert int(str(row["c_bigint"])) == 7000000000
    assert row["c_blob"], "blob column should round-trip as non-empty CSV/JSON wire value"
