...
def test_cbr_live_health_check(cbr_live_settings) -> None:
    from dpone.runtime.connectors.api.cbr import CbrConnector

    connector = CbrConnector(
        timeout=cbr_live_settings.timeout,
        retries=cbr_live_settings.retries,
        retry_delay=cbr_live_settings.retry_delay,
    )
    assert connector.health_check() is True


def test_cbr_live_fetch_small_window(cbr_live_settings) -> None:
    from dpone.runtime.connectors.api.cbr import CbrConnector
    ...
    rows = list(connector.get_resources(cbr_live_settings.resource, filters=_resolve_request(cbr_live_settings)))

    assert isinstance(rows, list)
    assert rows
    first = rows[0]
    assert first["valute_id"]
