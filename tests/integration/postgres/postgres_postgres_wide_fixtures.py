"""Wide PostgreSQL fixtures for postgres→postgres identity-profile live certification."""
...
WIDE_TABLE = "postgres_to_postgres_wide"
...
_SPECS: list[tuple[str, str, str, str]] = [
    ...
    ("c_blob", "BYTEA NULL", "bytea", "'\\x010203'::bytea"),
...
def _identity_resolve(pg_type: str) -> str:
    profile = build_default_profiles()[("postgres", "postgres")]
    return profile.resolve(pg_type).target_type
    ...
    for name, ddl, pg_type, seed in _SPECS:
        columns.append(
            WideColumn(
                ...
                pg_type=pg_type,
                seed_sql=seed,
                expected_pg_type=_identity_resolve(pg_type),
            )
        )
    return columns
