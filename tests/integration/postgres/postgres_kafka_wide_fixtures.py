"""Wide PostgreSQL fixtures for postgres→Kafka live certification.

No dedicated PairTypeMatrix profile (parity with mysql→kafka). Live assertions
validate JSON message payloads from CSV export wire rather than sink DDL.
"""
...
WIDE_TABLE = "postgres_to_kafka_wide"
...
def wide_columns() -> list[WideColumn]:
    return [
        WideColumn(name=name, pg_ddl=f'"{name}" {ddl}', pg_type=pg_type, seed_sql=seed)
