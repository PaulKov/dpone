"""Wide PostgreSQL fixtures for postgres→BigQuery pair-profile live certification."""
...
WIDE_TABLE = "postgres_to_bigquery_wide"
...
def wide_columns() -> list[WideColumn]:
    mapper = PostgresBigQueryTypeMapper()
    columns: list[WideColumn] = []
    for name, ddl, pg_type, seed in _SPECS:
        decision = mapper.resolve(pg_type)
        columns.append(
            WideColumn(
                name=name,
                pg_ddl=f'"{name}" {ddl}',
