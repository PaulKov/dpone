"""LoadConfig builders for postgres→BigQuery strategy live certification."""
...
from tests.integration.postgres.postgres_bigquery_wide_fixtures import SOURCE_SCHEMA, WIDE_TABLE
...
def _base(
    ...
    **kwargs,
    ...
    options = {
        "sink_type": "bigquery",
        # whole → FileExportArtifact → BQ direct load (no GCS). separate requires
        # an explicit object_storage bucket; vendor-live IT has none in this project.
    ...
    *, dataset: str, tmp_path: Path, table: str = WIDE_TABLE, source_schema: str = SOURCE_SCHEMA
    ...
    return _base(
        ...
        options={
            ...
            # Force COPY CSV (base64 BYTEA wrap). Streaming path does not encode BYTES.
            "delta_size_threshold": 0,
        },
    )
