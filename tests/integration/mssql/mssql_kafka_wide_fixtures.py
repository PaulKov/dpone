"""Wide MSSQL fixtures for mssql→Kafka live certification.

No dedicated PairTypeMatrix profile (parity with mysql/postgres→kafka). Live
assertions validate JSON message payloads from CSV export wire.
"""
...
WIDE_TABLE = "mssql_to_kafka_wide"
...
def wide_columns() -> list[WideColumn]:
    return [
        WideColumn(name=name, mssql_ddl=f"[{name}] {ddl}", mssql_type=mssql_type, seed_sql=seed)
