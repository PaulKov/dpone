"""Wide MSSQL fixtures for mssql→BigQuery pair-profile live certification."""
...
WIDE_TABLE = "mssql_to_bigquery_wide"
...
_SPECS: list[tuple[str, str, str, str]] = [
    ...
    ("c_blob", "varbinary(16) NULL", "varbinary(16)", "0x010203"),
...
def wide_columns() -> list[WideColumn]:
    mapper = MSSQLBigQueryTypeMapper()
    columns: list[WideColumn] = []
    for name, ddl, mssql_type, seed in _SPECS:
        decision = mapper.resolve(mssql_type)
        columns.append(
            WideColumn(
                name=name,
