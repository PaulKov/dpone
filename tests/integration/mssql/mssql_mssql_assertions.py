"""Assertions for mssql→mssql wide live certification."""
...
from tests.integration.mssql.mssql_mssql_strategy_configs import TARGET_SCHEMA
from tests.integration.mssql.mssql_mssql_wide_fixtures import WideColumn

# SQL Server INFORMATION_SCHEMA.DATA_TYPE families for identity targets.
_MSSQL_TYPE_ALIASES = {
    "int": {"int"},
    "smallint": {"smallint"},
    "bigint": {"bigint"},
    ...
    *,
    ...
    rows = mssql.get_records(
        ...
        SELECT id, c_name, c_varchar, c_bool, c_decimal, c_bigint,
               CONVERT(varchar(max), c_varbinary, 2) AS blob_hex
    ...
    assert str(row["blob_hex"]).lower() == "010203", f"c_varbinary hex: got {row['blob_hex']!r}"
