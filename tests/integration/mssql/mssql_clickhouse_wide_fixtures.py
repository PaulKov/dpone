"""Wide MSSQL fixtures for mssql→ClickHouse pair-profile live certification."""
...
WIDE_TABLE = "mssql_to_clickhouse_wide"
SOURCE_SCHEMA = "dpone_src"

# Round-tripable types aligned with mssql_to_clickhouse_lossless_v2 (excl. contract-only types).
...
def wide_columns() -> list[WideColumn]:
    mapper = MssqlClickHouseTypeMapper(MssqlClickHouseTypePolicy(binary_encoding="hex"))
    ...
    for name, ddl, mssql_type, seed in _SPECS:
        columns.append(
            WideColumn(
                ...
                seed_sql=seed,
                expected_ch_type=mapper.resolve_column(name, mssql_type).target_type,
            )
        )
    return columns
