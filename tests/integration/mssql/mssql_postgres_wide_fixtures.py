"""Wide MSSQL fixtures for mssql→Postgres pair-profile live certification."""
...
WIDE_TABLE = "mssql_to_postgres_wide"
SOURCE_SCHEMA = "dpone_src"

# Round-tripable types aligned with mssql_to_postgres_native_v1.
...
def wide_columns() -> list[WideColumn]:
    mapper = MSSQLPostgresTypeMapper()
    ...
    for name, ddl, mssql_type, seed in _SPECS:
        columns.append(
            WideColumn(
                ...
                seed_sql=seed,
                expected_pg_type=mapper.resolve(mssql_type).target_type,
            )
        )
    return columns
