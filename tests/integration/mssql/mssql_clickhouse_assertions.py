"""Assertions for mssql→ClickHouse wide live certification."""
...
from tests.integration.mssql.mssql_clickhouse_strategy_configs import TARGET_DATABASE
from tests.integration.mssql.mssql_clickhouse_wide_fixtures import WideColumn
...
_CH_TYPE_ALIASES = {
    ...
    "string": {"String"},
    "uuid": {"UUID"},
    "datetime64(3)": {"DateTime64(3)", "DateTime64(6)", "DateTime64(7)", "DateTime64(0)"},
    "datetime64(7)": {"DateTime64(7)", "DateTime64(6)", "DateTime64(3)", "DateTime64(0)"},
}
...
def assert_typed_spot_checks(
    ...
    *,
    ...
    rows = clickhouse.get_records(
        f"""
        SELECT id, c_name, c_varchar, c_bool, c_decimal, c_bigint, c_varbinary
        FROM `{database}`.`{table}`
        WHERE id = {row_id}
        """,
        as_dict=True,
    ...
    binary = row["c_varbinary"]
    if isinstance(binary, (bytes, bytearray, memoryview)):
        assert bytes(binary) in {b"\x01\x02\x03", b"010203"}
    else:
        text = str(binary)
        assert text in {"010203", "\x01\x02\x03"} or "010203" in text.lower().replace("0x", "")
